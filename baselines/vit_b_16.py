...
dataset = {"train": FashProd(transform=img_preprocess, split="train"),
           "val": FashProd(transform=img_preprocess, split="val"),
           "test": FashProd(transform=img_preprocess, split="test")}
...
def get_weights():
    ...
    counts = [num_df_data / float(count) for count in counts]
...
model = vit_b_16(num_classes=num_classes, image_size=384).to(device)
...
for e in tqdm(range(n_epoch), desc="Training model..."):  # progress bar for counting epochs
    ...
    train_count = 0
    train_total = 0
    for train_x, train_y in tqdm(train_loader, desc=f"EPOCH {e + 1} - training"):  # progress bar for training
        optimizer.zero_grad()
    ...
    val_total = 0
    with torch.no_grad():
        for val_x, val_y in tqdm(val_loader, desc=f"EPOCH {e + 1} - validation"):  # progress bar for validation
...
fig = plt.figure()
plt.plot(range(1, n_epoch + 1), y=train_accs, label="Training accuracy")
plt.plot(range(1, n_epoch + 1), y=val_accs, label="Validation accuracy")
plt.legend()
...
if os.path.exists(f"models/{model_name}.pth"):
    model.load_state_dict(torch.load(f"models/{model_name}"), map_location="cuda")
model.eval()
predictions = []
targets = []
with torch.no_grad():
    ...
    test_count = 0
    test_total = 0
    for test_x, test_y in tqdm(val_loader, desc="Predicting..."):
        test_pred = model(test_x)
        test_loss += loss_fn(test_pred, test_y.long()).item() * len(test_y)
        test_count += (test_pred.softmax(1).argmax(1) == test_y).sum().item()
