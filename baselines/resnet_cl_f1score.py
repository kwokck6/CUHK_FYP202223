...
n_epoch = 10
batch_size = 3
model_name = "resnet50_cl_f1score"
...
dataset = {"train": ShopeeCL(img_transform=img_preprocess, split="train"),
           "val": ShopeeCL(img_transform=img_preprocess, split="val"),
           "test": ShopeeCL(img_transform=img_preprocess)}
...
model = resnet50(num_classes=128).to(device)  # embed_size = 128
optimizer = AdamW(model.parameters(), lr=1e-4)
scheduler = ReduceLROnPlateau(optimizer, mode="min", patience=2, verbose=True)
loss_fn = ContrastiveLoss(margin=1.0).to(device)
...
for e in tqdm(range(n_epoch), desc="Training model..."):  # progress bar for counting epochs
    ...
    train_total = 0
    train_preds = []
    for (train_img1, train_img2), train_text, (train_y1, train_y2) in tqdm(train_loader, desc=f"EPOCH {e + 1} - training"):  # progress bar for training
        optimizer.zero_grad()
    ...
    val_preds = []
    with torch.no_grad():
        for (val_img1, val_img2), val_text, (val_y1, val_y2) in tqdm(val_loader, desc=f"EPOCH {e + 1} - validation"):  # progress bar for validation
            val_pred1 = model(val_img1.to(device))
            val_pred2 = model(val_img2.to(device))
            val_y = (val_y1 == val_y2).long().to(device)
            val_loss += loss_fn(val_pred1, val_pred2, val_y).item() * len(val_y)
            val_total += len(val_y)
...
test_preds = []
with torch.no_grad():
    for (val_img1, val_img2), val_text, (val_y1, val_y2) in tqdm(val_loader, desc="Retrieving best embedding..."):
    ...
    test_loss = 0.
    test_total = 0
    for (test_img1, test_img2), test_text, (test_y1, test_y2) in tqdm(test_loader, desc="Predicting..."):
        test_pred1 = model(test_img1.to(device))
        test_pred2 = model(test_img2.to(device))
        test_y = (test_y1 == test_y2).to(device).long()
        test_loss += loss_fn(test_pred1, test_pred2, test_y).item() * len(test_y)
        test_total += len(test_y)
        predictions.append(test_pred1.detach().cpu().numpy())
        targets.append(test_y1)
