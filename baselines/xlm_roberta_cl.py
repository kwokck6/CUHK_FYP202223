...
model = XLMRobertaForSequenceClassification.from_pretrained('roberta-base', num_labels=128).to(device)  # embed_dim = 128
...
for e in tqdm(range(n_epoch), desc="Training model..."):  # progress bar for counting epochs
    ...
    for train_img1, (train_text1, train_text2), (train_y1, train_y2) in tqdm(train_loader, desc=f"EPOCH {e + 1} - training"):  # progress bar for training
        ...
        loss = loss_fn(train_pred1.logits, train_pred2.logits, train_y)
        train_loss += loss.item() * len(train_y)
        train_total += len(train_y)
        train_preds.append(train_pred1.logits.detach().cpu().numpy())
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        for val_img, (val_text1, val_text2), (val_y1, val_y2) in tqdm(val_loader, desc=f"EPOCH {e + 1} - validation"):  # progress bar for validation
            ...
            val_preds.append(val_pred1.logits.detach().cpu().numpy())
            val_loss += loss_fn(val_pred1.logits, val_pred2.logits, val_y).item() * len(val_y)
            val_total += len(val_y)
...
with torch.no_grad():
    ...
    for test_img, (test_text1, test_text2), (test_y1, test_y2) in tqdm(test_loader, desc="Predicting..."):
        ...
        test_loss += loss_fn(test_pred1.logits, test_pred2.logits, test_y.long().to(device)).item() * len(test_y)
        test_total += len(test_y)
        predictions.append(test_pred1.logits.detach().cpu().numpy())
